...
namespace LiaoChengZYSI
{
    public class Process : Neusoft.HISFC.BizProcess.Interface.FeeInterface.IMedcare
    {
        ...
        #region 实现接口成员方法，但这些方法没有被使用,至少没有被医保接口对象使用
        ...
        public bool IsInBlackList(Neusoft.HISFC.Models.RADT.PatientInfo info)
        {
            return true;
        }

        public bool IsInBlackList(Neusoft.HISFC.Models.Registration.Register reg)
        {
            return true;
        }
        ...
        public int BalanceInpatient(Neusoft.HISFC.Models.RADT.PatientInfo p, ref System.Collections.ArrayList feeDetails)
        {
            ...
            {
                //balanceNO = "0";
                p.SIMainInfo.BalNo = "1";
            }
            else
            {
                isModify = true;
                p.SIMainInfo.BalNo = balanceNO;
            }
            ...
            p.SIMainInfo.OperDate = p.PVisit.InTime;
            p.SIMainInfo.OperInfo.ID = this.localManager.Operator.ID;//获取当前操作员信息
            p.SIMainInfo.BusinessSequence = "";//this.BusinessSequenceZY;
            ...
            {
                returnValue = this.localManager.InsertSIMainInfo(p);
            }
            if (returnValue < 0)
            {
                this.errText = this.localManager.Err;
        ...
        private int connInit()
        {
            ...
            if (this.sourceObject.ToString() == "LiaoChengZYSI.Control.ucCompare" || this.sourceObject.ToString() == "LiaoChengZYSI.Control.frmUploadCheckedInfo, Text: 医保费用上传" || this.sourceObject.ToString() == "LiaoChengZYSI.Control.frmUploadFeeDetails, Text: 医保批量上传")
            {
                ...
                returnValue = seiInterfaceProxy_new.request_service("init");

                if (returnValue != 0)
                {
                    this.errText = "登陆医保数据库失败 \n错误代码：" + returnValue + "\n 错误信息：" + this.seiInterfaceProxy_new.get_errtext();
                    return -1;
                }
                isInit = true;
                #endregion
        ...
        public int UploadFeeDetailsInpatient(Neusoft.HISFC.Models.RADT.PatientInfo patient, ref System.Collections.ArrayList feeDetails)
        {
            ...
            #region 保存到医保中心
            seiInterfaceProxy_new.resetvar();
            seiInterfaceProxy_new.putvarstring("ysbm", centerDoctID);
            seiInterfaceProxy_new.putvardatetime("date", dt);
        ...
        public int DeleteUploadedFeeDetailsAllInpatient(Neusoft.HISFC.Models.RADT.PatientInfo patient)
        {
            ...
            returnValue = seiInterfaceProxy_new.request_service("delete_all_fypd");
            if (returnValue != 0)
            {
        ...
        public int QueryUndrugLists(ref System.Collections.ArrayList comparedList, string pactCode)
        {
            ...
            #region 调用下载已对照项目的接口
            this.seiInterfaceProxy_new.resetvar();
            this.seiInterfaceProxy_new.putvarstring("sbjgbh", "37150205");
            this.seiInterfaceProxy_new.putvarstring("filename", filePathZL);
            this.seiInterfaceProxy_new.putvardec("filetype", 1);
            this.seiInterfaceProxy_new.putvardec("has_head", 0); //包含表头
            int returnValue = this.seiInterfaceProxy_new.request_service("down_yyxm");
            if (returnValue != 0)
            {
        ...
        public int QueryDrugLists(ref System.Collections.ArrayList drugLists, string pactCode)
        {
            ...
            #region 调用下载医保中心项目的接口
            this.seiInterfaceProxy_new.resetvar();
            this.seiInterfaceProxy_new.putvarstring("sbjgbh", "37150205");
            this.seiInterfaceProxy_new.putvarstring("filename", filePathZL);
            this.seiInterfaceProxy_new.putvarstring("filename2", ""); //自负比例先不下载
            this.seiInterfaceProxy_new.putvardec("filetype", 1);
            this.seiInterfaceProxy_new.putvardec("has_head", 0); //包含表头
            int returnValue = this.seiInterfaceProxy_new.request_service("down_ml");
            if (returnValue != 0)
            {
